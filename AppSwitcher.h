...
public:
  ...
  void setHoldKey(Key key) { hold_key_ = key; }
  void setHostHoldKey(Key key) { host_hold_key_ = key; }
  void setHostForwardKey(Key key) { host_forward_key_ = key; }
  void setHostReverseKey(Key key) { host_reverse_key_ = key; }
    ...
    if (active_) {
      ...
      bool other_key_pressed =
          ...
          !(key == Key(kaleidoscope::ranges::APPSWITCHER_F) ||
            key == Key(kaleidoscope::ranges::APPSWITCHER_R) ||
            key == hold_key_);
